 ...
 * 上下文中展示的加载进度View
 */
class ContentLoadingView(context: Context, attrs: AttributeSet?, defStyleAttr: Int):
...
//        setImageDrawable(progressDrawable)
    ...
    fun show() {
        ...
        log("show() -> postedShow: $postedShow")
        // 重置启动时间
        startTime = -1
        dismissed = false
    ...
    fun hide() {
        ...
        if (startTime < 0) {
            log("hide() -> startTime < 0")
            hideView()
        } else if (diff >= MIN_SHOW_TIME) {
            log("hide() -> diff >= MIN_SHOW_TIME")
            animationToHide()
        } else {
            log("hide() -> postedHide")
    ...
    override fun onAttachedToWindow() {
        ...
        progressDrawable.stop()
        if (postedShow) {
            log("onAttachedToWindow() -> show()")
            postedShow = false
            show()
            return
        }
        log("onAttachedToWindow() -> hideView()")
        postedHide = false
        hideView()
    }
    ...
    class ProgressDrawable: Drawable(), Animatable,
        ...
        override fun onBoundsChange(bounds: Rect?) {
            ...
            radius = bounds.width().coerceAtMost(bounds.height()) * 0.5F
        ...
        override fun onAnimationStart(animation: Animator?) {
            ...
            colorIndex %= colorList.size
            paint.color = colorList[colorIndex]
            log("onAnimationStart($colorIndex)")
        }
